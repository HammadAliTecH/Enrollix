...
// -----------------------------------------------------------
  document.querySelectorAll('.view_enroll_details').forEach(button => {
    button.addEventListener('click', function () {
        const enrollmentData = this.dataset.enrollment;   // data-role-name attribute se
        const courseData = this.dataset.course;   // data-role-name attribute se
        ...
        // Input fields mein value set karo
        document.getElementById('enroll_stu_image').src = '/students_data/students_profile_images/' + enrollment_filter_data.image;
        document.getElementById('enroll_stu_name').innerText = enrollment_filter_data.name;
        document.getElementById('enroll_stu_gender').innerText = enrollment_filter_data.gender;
        document.getElementById('enroll_stu_age').innerText = enrollment_filter_data.age;
        document.getElementById('enroll_stu_cnic_number').innerText = enrollment_filter_data.cnic_number;
        document.getElementById('enroll_stu_father_name').innerText = enrollment_filter_data.father_name;
        document.getElementById('enroll_stu_father_cnic').innerText = enrollment_filter_data.father_cnic;
        document.getElementById('enroll_stu_father_occupation').innerText = enrollment_filter_data.father_occupation;



        document.getElementById('enroll_stu_phone_number').innerText = enrollment_filter_data.contact_number;
        document.getElementById('enroll_stu_father_cell_number').innerText = enrollment_filter_data.father_cell_number;
        document.getElementById('enroll_stu_email').innerText = enrollment_filter_data.email;

       document.getElementById('enroll_stu_current_education').innerText = enrollment_filter_data.enrolled_program;
       document.getElementById('enroll_stu_recent_qualification').innerText = enrollment_filter_data.recent_education;
       document.getElementById('enroll_stu_marks').innerText = enrollment_filter_data.marks;
       document.getElementById('enroll_stu_institute_name').innerText = enrollment_filter_data.educational_place;

       document.getElementById('enroll_stu_image_2').href = '/students_data/students_profile_pictures/' + enrollment_filter_data.image;
       document.getElementById('enroll_stu_cnic_document').href = '/students_data/students_cnic_documents/' + enrollment_filter_data.cnic_document;

        document.getElementById('enroll_course_name').innerText = course_filter_data.name;
        document.getElementById('enroll_course_instructor').innerText = course_filter_data.user.name;
        document.getElementById('enroll_course_duration').innerHTML = '<span class="badge text-bg-success">'+course_filter_data.duration+'</span>';
        document.getElementById('enroll_course_payment_plan').innerText = course_filter_data.payment_type;
        document.getElementById('enroll_course_installments').innerText = course_filter_data.total_installments;
...
$(document).on('click', '.plan_detail_model', function () {
    ...
    // Button ke data-id se student_course_id lo
    let student_course_id = $(this).data('id');

    // Purana content clear karo taake pichli baar ka data na dikhe
    ...
    $.ajax({
        ...
        type: "GET", // route GET hai, controller bhi GET expect karta hai
        headers: {
        ...
        success: function (response) {
            ...
            if (response.length === 0) {
                html = `<div class="alert alert-warning mb-0">No payment plan found for this student.</div>`;
            } else {

                // Har installment ke liye ek table banao
            ...
            success: function (response) {
                ...
                $.each(response, function (i, student) {
                    ...
                    if (!student.student_courses || student.student_courses.length === 0) {
                        html += `<div class="alert alert-warning mb-4">No enrolled course found for this student.</div>`;
                        return; // continue jaisa
                    }

                    // LEVEL 2: har student_course (enrollment)
                    $.each(student.student_courses, function (j, sc) {
                        ...
                        if (paymentPlans.length === 0) {
                            html += `<div class="alert alert-warning mb-4">No payment plan found for this course.</div>`;
                            return; // continue jaisa
                        }

                        // LEVEL 3: har payment_plan (installment)
...
// Kyunki pay_installment_btn dynamically (AJAX se) inject ho raha hai,
// isliye $(document).on() use karna zaroori hai — direct .click() kaam nahi karega

$(document).on('click', '.pay_installment_btn', function () {

    // Clicked button se data-id nikalo
    let plan_id = $(this).data('id');

    console.log('Selected Payment Plan ID:', plan_id);

    // Modal ke hidden input mein set karo
